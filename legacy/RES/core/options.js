...
RESUtils.options.listTypes['subreddits'] = {
	...
	onCachedResult: function onCachedResult(response) {
		var names = response.names,
		    results = names.map(function (name) {
			return {
				id: name,
	...
	sanitizeValues: function sanitizeValues() {
		var values = Array.prototype.slice.call(arguments).reduce(function (a, b) {
			return a.concat(b);
		}, []);
...
$.extend(RESUtils.options, {
	resetModulePrefs: function resetModulePrefs() {
		this.setModulePrefs({});
	},
	getAllModulePrefs: function getAllModulePrefs(force) {
		var storedPrefs;
		...
		for (var module in modules) {
			...
			} else if (!modules[module].disabledByDefault && (storedPrefs[module] === undefined || storedPrefs[module] === null || module.alwaysEnabled)) {
	...
	getModulePrefs: function getModulePrefs(moduleID) {
		if (moduleID) {
	...
	setModulePrefs: function setModulePrefs(prefs) {
		if (prefs !== null) {
	...
	getModuleIDsByCategory: function getModuleIDsByCategory(category) {
		var moduleList = Object.getOwnPropertyNames(modules).filter(function (moduleID) {
	...
	enableModule: function enableModule(moduleID, onOrOff) {
	...
	setOption: function setOption(moduleID, optionName, optionValue) {
		...
		if (optionValue === '') {
			saveOptionValue = '';
		} else if (isNaN(optionValue) || typeof optionValue === 'boolean' || (typeof optionValue === 'undefined' ? 'undefined' : _typeof(optionValue)) === 'object') {
	...
	saveModuleOptions: function saveModuleOptions(moduleID, newOptions) {
		function minify(obj) {
			var min = {};
			if (obj) {
				for (var key in obj) {
	...
	loadOptions: function loadOptions(moduleID) {
		...
		if (needsStorage) {
			return RESStorage.loadItem('RESoptions.' + moduleID).then(RESUtils.options.getOptions.bind(RESUtils.options, moduleID));
		} else {
			return $.Deferred().resolve(modules[moduleID].options);
		}
	},
	getOptionsFirstRun: {},
	getOptions: function getOptions(moduleID, keepObsoleteOptions) {
	...
	removeObsoleteOptions: function removeObsoleteOptions(moduleID) {
