...
var RESOptionsMigrate = {
	migrations: [{
		...
		go: function go() {
			...
			RESOptionsMigrate.migrators.generic.moveOption('styleTweaks', 'lightOrDark', 'nightMode', 'nightModeOn', RESOptionsMigrate.migrators.specific.nightModeOn);
			RESOptionsMigrate.migrators.generic.moveOption('styleTweaks', 'useSubredditStyleInDarkMode', 'nightMode', 'useSubredditStyles');
			RESOptionsMigrate.migrators.generic.moveStorageToOption('RESmodules.styleTweaks.nightModeWhitelist', 'nightMode', 'subredditStylesWhitelist', RESOptionsMigrate.migrators.specific.subredditStylesWhitelist);
		...
		go: function go() {
			...
			RESOptionsMigrate.migrators.generic.moveOption('settingsNavigation', 'showAdvancedOptions', 'settingsNavigation', 'showAllOptions', true);
		...
		versionNumber: '4.7.0',
		go: function go() {
			RESOptionsMigrate.migrators.generic.forceUpdateOption('orangered', 'showUnreadCount', function (value) {
				return value === false;
			});
	...
	migrators: {
		generic: {
			updateOption: function updateOption(moduleID, optionName, formerDefaultValue, valueOrFunction) {
				try {
			...
			forceUpdateOption: function forceUpdateOption(moduleID, optionName, valueOrFunction) {
			...
			moveOption: function moveOption(oldModuleID, oldOptionName, newModuleID, newOptionName, valueOrFunction) {
				try {
			...
			moveStorage: function moveStorage(oldKey, newKey, valueOrFunction) {
			...
			moveStorageToOption: function moveStorageToOption(oldKey, newModuleID, newOptionName, valueOrFunction) {
				var oldValue = RESStorage.getItem(oldKey);
			...
			selectDistinctRowsByColumnIndex: function selectDistinctRowsByColumnIndex(distinctColumns, rows) {
				/**
    ...
    */
			...
			_testSelectDistinctRowsByColumnIndex: function _testSelectDistinctRowsByColumnIndex() {
				// TODO: move to qunit file
				var f = RESOptionsMigrate.migrators.generic.selectDistinctRowsByColumnIndex,
				    distinct = [0, 1],
				    rows = [['for matching', 'values', 'keep the first row found'], ['for matching', 'values', 'drop other rows'], ['for non-matching', 'values', 'keep the only row found']],
				    expected = [['for matching', 'values', 'keep the first row found'], ['for non-matching', 'values', 'keep the only row found']],
		...
		specific: {
			nightModeOn: function nightModeOn(value) {
				return value === 'dark';
			},
			subredditStylesWhitelist: function subredditStylesWhitelist(value) {
				var parsedValue = safeJSON.parse(value);
				return parsedValue.join(',');
			},
			colorCommentScore: function colorCommentScore(value) {
				...
				if (!value) {
					RESOptionsMigrate.showNotification('voteEnhancements', 'colorCommentScore', 'The Vote Enhancements colorCommentScore feature has been turned off.');
				}

				// 4.5.0.1 migration
				return value ? 'automatic' : 'none';
			},
			removeUndefinedUnlessKeyword: function removeUndefinedUnlessKeyword(values) {
			...
			updateFadeSpeed: function updateFadeSpeed(value) {
				if (value < 0 || value > 1) {
					return '0.7';
				} else {
			...
			commentToolsDefaultMacros: function commentToolsDefaultMacros(value) {
				value = value || [];
				value = [['reddiquette', '[reddiquette](/wiki/reddiquette) '], ['Promote RES', '[Reddit Enhancement Suite](http://redditenhancementsuite.com "also /r/Enhancement") '], ['Current timestamp', '{{now}} ']].concat(value);
				return value;
			},
			updateTagStorage: function updateTagStorage() {
			...
			clearOldLocalStorage: function clearOldLocalStorage(keys) {
			...
			updateTagStorageCaseInsensitive: function updateTagStorageCaseInsensitive(tags) {
	...
	showNotification: function showNotification(moduleID, optionName, message) {
		message = RESUtils.firstValid(message, '') + '<br><br>You can change this option ' + modules['settingsNavigation'].makeUrlHashLink('voteEnhancements', 'colorCommentScore', 'in the RES settings console.');
	...
	getMigrationStartIndex: function getMigrationStartIndex() {
	...
	getLastMigratedVersion: function getLastMigratedVersion() {
	...
	setLastMigratedVersion: function setLastMigratedVersion(value) {
		if (value) {
			RESStorage.setItem('RESOptionsVersion', value);
		}
	},

	getVersionNumbers: function getVersionNumbers() {
		return RESOptionsMigrate.migrations.map(function (migration) {
			return migration.versionNumber;
		});
	},
	...
	optionMatchesFormerDefaultValue: function optionMatchesFormerDefaultValue(option, formerDefaultValue) {
		if (!option) {
			option = {
				type: 'legacy',
				value: undefined
	...
	updateValue: function updateValue(oldValue, valueOrFunction) {
		var newValue;
