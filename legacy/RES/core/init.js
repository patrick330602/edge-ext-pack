...
RESUtils.init = function () {
	...
	if (/\/toolbar\/toolbar\?id/i.test(location.href) || /comscore-iframe/i.test(location.href) || /(?:static|thumbs|blog|code)\.reddit\.com/i.test(location.hostname) || /^[www\.]?(?:i|m)\.reddit\.com/i.test(location.href) || /\.(?:compact|mobile)$/i.test(location.pathname) || /metareddit\.com/i.test(location.href)) {
		return;
	}
	...
	RESUtils.init.await('metadata').done(function () {
		RESUtils.bodyClasses.init();RESUtils.bodyClasses.add();
	});
	...
	RESUtils.deferred.all([RESUtils.init.await('metadata'), RESUtils.init.await.bodyReady()]).done(RESUtils.init._reportVersion);
	RESUtils.init.await('storage').fail(RESUtils.init._localStorageFailed);

	RESUtils.init.await.go().fail(RESUtils.init._showErrors);
	RESUtils.deferred.all([RESUtils.init.await.bodyReady(), RESUtils.init.await.go()]).done(RESUtils.initObservers);
};
...
RESUtils.init._watch.storage = function () {
	var def = RESStorage.setup().then(RESOptionsMigrate.migrate);
...
RESUtils.init._watch.initReadyCheck = function () {
	...
	if (!RESStorage.isReady || typeof document.body === 'undefined' || !document.html || typeof document.html.classList === 'undefined') {
...
RESUtils.init.await.library = function (libraryID) {
	return RESUtils.init.await._deferred('sourceLoaded').then(function () {
...
RESUtils.init._loadModules = function () {
	...
	RESUtils.init._queueModulesStage('loadLibraries', function (stage, module, moduleID) {
		return RESUtils.init.await._deferred('sourceLoaded').then(RESUtils.init._runModuleStage.bind(this, stage, module, moduleID));
	});

	RESUtils.init._queueModulesStage('loadDynamicOptions', function (stage, module, moduleID) {
		return RESUtils.init.await._deferred('loadLibraries').then(RESUtils.init._runModuleStage.bind(this, stage, module, moduleID));
	});
	RESUtils.init._queueModulesStage('options', function (stage, module, moduleID) {
		return RESUtils.init.await.loadDynamicOptions(moduleID).then(RESUtils.options.loadOptions.bind(this, moduleID));
	});
	RESUtils.init._queueModulesStage('beforeLoad', function (stage, module, moduleID) {
		return RESUtils.deferred.all([RESStorage.loadItem('RES.modulePrefs'), RESUtils.init.await.options(moduleID), RESUtils.init.await('headReady')]).then(RESUtils.init._runModuleStage.bind(this, stage, module, moduleID));
	});
	RESUtils.init._queueModulesStage('addOptionsBodyClasses', function (stage, module, moduleID) {
		return RESUtils.deferred.all([RESStorage.loadItem('RES.modulePrefs'), RESUtils.init.await.options(moduleID)]).then(RESUtils.init._runModuleStage.bind(this, stage, module, moduleID));
	});
	RESUtils.init._queueModulesStage('go', function (stage, module, moduleID) {
		return RESUtils.deferred.all([RESUtils.init.await.beforeLoad(moduleID), RESUtils.init.await('bodyReady')]).then(RESUtils.init._runModuleStage.bind(this, stage, module, moduleID));
	});

	RESUtils.init._queueModulesStage('afterLoad', function (stage, module, moduleID) {
		return RESUtils.deferred.all([RESUtils.init.await.go(moduleID), RESUtils.init.await('documentContentLoaded')]).then(RESUtils.init._runModuleStage.bind(this, stage, module, moduleID));
	});
...
RESUtils.init._showErrors = function () {
	...
	for (var type in deferreds) {
		if (!(deferreds.hasOwnProperty(type) && _typeof(deferreds[type]) === 'object')) continue;
...
RESUtils.init._homePage = function () {
	if (location.href.indexOf('reddit.honestbleeps.com/download') !== -1 || location.href.indexOf('redditenhancementsuite.com/download') !== -1) {
