...
addModule('showParent', function (module, moduleID) {
	...
	module.go = function () {
		if (this.isEnabled() && this.isMatchURL()) {
			$('body').on('mouseenter', '.comment .buttons :not(:first-child) .bylink', function (e) {
				modules['hover'].infocard('showParent').target(this).options({
				...
				}).populateWith(module.showCommentHover).begin();
			});
		}
	};
	function handleVoteClick(evt) {
		...
		var $targetButton = $('.content .thing.id-' + id).children('.midcol').find('.arrow.' + direction + ', .arrow.' + direction + 'mod');
		...
		var removeClickHandlers = function removeClickHandlers(event) {
		...
		$midcol.find('.up, .upmod').toggleClass('upmod', newDir === 1).toggleClass('up', newDir !== 1);
		$midcol.find('.down, .downmod').toggleClass('downmod', newDir === -1).toggleClass('down', newDir !== -1);
	}
...
//# sourceMappingURL=showParent.js.map
