...
addLibrary('mediaHosts', 'onedrive', {
	...
	fetchJson: function fetchJson(elem, href, def) {
				...
				try {
					...
					if (json.folder !== undefined) {
						...
						RESEnvironment.ajax({
							...
							url: folderUrl,
							aggressiveCache: true,
							onload: function onload(folderResponse) {
							...
							onerror: function onerror(folderResponse) {
								def.reject();
							}
						});
					} else {
	...
	resolveElement: function resolveElement(elem, info) {
	...
	handleInfo: function handleInfo(elem, info) {
		...
		if (info.isShort) {
			...
			if (elem === null) def.reject();else def.resolve(elem);
		}
		return def.promise();
	}
});
