...
addLibrary('mediaHosts', 'imgur', {
	...
	_deferredImages: {},
	_deferredImageQueue: [],
	_handleImage: function _handleImage(hash, url) {
		var siteMod = modules['showImages'].siteModules['imgur'],
		    def;
	...
	_batchHandleImages: function _batchHandleImages() {
		var siteMod = modules['showImages'].siteModules['imgur'],
		...
		queue.forEach(function (item) {
			...
			def.done(item.deferred.resolve).fail(item.deferred.reject);
		});
	},

	_hashMockAPI: function _hashMockAPI(hash, url) {
		var siteMod = modules['showImages'].siteModules['imgur'],
		    def = $.Deferred(),
		    matches,
		    extension,
	...
	_handleImageCollection: function _handleImageCollection(hashes, url) {
		var siteMod = modules['showImages'].siteModules['imgur'],
		    def,
		    expandoInfo = {
			is_album: true
	...
	_handleSingleImage: function _handleSingleImage(elem, info) {
		var link = info.link;
	...
	_handleAlbum: function _handleAlbum(elem, info) {
		var siteMod = modules['showImages'].siteModules['imgur'],
		    deferred,
		    expandoInfo = {},
	...
	_handleGifv: function _handleGifv(elem, info) {
		var siteMod = modules['showImages'].siteModules['imgur'],
		    expandoInfo = {};
	...
	_generateGifv: function _generateGifv(elem, info) {
