...
addModule('settingsConsole', function (module, moduleID) {
	...
	var modalOverlay, keyCodeModal, RESConsoleContainer, moduleOptionsScrim, saveButton, moduleToggle, captureKey, captureKeyID, currentModule;
	...
	function _create(template) {
		...
		window.addEventListener('keydown', function (e) {
			if (captureKey && e.keyCode !== 16 && e.keyCode !== 17 && e.keyCode !== 18) {
				...
				var keyArray;
				if (e.keyCode === 8) {
					// backspace, we disable the shortcut
	...
	function populateCategories() {
		...
		var categorySortSpec = RESMetadata.categories.slice(0).map(function (a) {
		...
		if (splice === -1) {
			...
			categoryNames = presortedCategories.slice(0, splice).concat(unsortedCategories).concat(presortedCategories.slice(splice + 1));
		}
    ...
    */
	...
	function drawSettingsConsole() {
		...
		$(thisToggle).toggle(!modules[moduleID].alwaysEnabled).toggleClass('enabled', modules[moduleID].isEnabled()).data('module', moduleID);
		...
		thisToggle.addEventListener('click', function (e) {
			...
			$(moduleOptionsScrim).toggleClass('visible', !enable);
	...
	function drawConfigOptions(moduleID) {
		...
		$(moduleToggle).toggle(!modules[moduleID].alwaysEnabled).toggleClass('enabled', modules[moduleID].isEnabled()).data('module', moduleID);
	...
	function stageCurrentModuleOptions() {
		...
		$(panelOptionsDiv).find('.optionContainer:not(.specialOptionType)').find('input, textarea').each(function (i, input) {
	...
	module.open = function (moduleIdOrCategory) {
		...
		$(RESConsoleContainer).on('change', RESUtils.debounce.bind(RESUtils, AUTOSTAGE_DEBOUNCE_NAME, 300, stageCurrentModuleOptions)).on('keyup', RESUtils.debounce.bind(RESUtils, AUTOSTAGE_DEBOUNCE_NAME, 1000, stageCurrentModuleOptions));
	};
	...
	function openCategoryPanel(categories) {
		...
		if (selected.filter('.active').length === 0) {
			// if a category isn't already open
	...
	module.getOptionValue = function (moduleID, optionKey) {
		...
		if (optionInput) {
			...
			console.error('Cannot get a value for ' + moduleID + '.' + optionKey + ' because the HTML element does not exist.');
			return null;
		}
	};
	...
	module.setOptionValue = function (moduleID, optionKey, value) {
		...
		if (optionInput) {
			...
			console.error('Cannot set a value for ' + moduleID + '.' + optionKey + ' because the HTML element does not exist.');
		}
	};
	...
	function drawOptionBuilder(options, moduleID, optionName) {
		...
		$addRowButton.text(option.addItemText || '+add item').on('click', function (e) {
	...
	function drawBuilderItem(data, config) {
		...
		var $handle = $('<div>').addClass('res-icon-button res-icon handle').html('&#xF0AA;').attr('title', 'drag and drop to move this filter');

		var $deleteButton = $('<div>').addClass('res-icon-button res-icon deleteButton').html('&#xF056;').attr('title', 'remove this filter').on('click', function (e) {
		...
		var $editButton = $('<div>').addClass('res-icon-button res-icon').html('&#xF061;').attr('title', 'copy and share, or update your settings with a new version').on('click', function (e) {
			var data = readBuilderItem($(this).closest('.builderItem'), config),
			    json = prompt('Copy this and share it, or paste a new version and update your settings', JSON.stringify(data));
			if (json !== null) {
		...
		$body.append([$('<div class="builderItemControls">').append($handle), $('<div class="builderItemControls builderTrailingControls">').append($editButton, $deleteButton), $header, $versionField, $wrapper]);
	...
	function drawBlockControls() {
		var $handle = $('<div>').addClass('res-icon-button res-icon handle').html('&#xF0AA;').attr('title', 'drag and drop to move this condition');

		return $('<div class="builderControls">').append($handle);
	}

	function drawBlockTrailingControls() {
		var $deleteButton = $('<div>').addClass('res-icon-button res-icon deleteButton').html('&#xF056;').attr('title', 'remove this condition');
	...
	var builderFields = {
		multi: {
			draw: function draw(data, blockConfig, config) {
			...
			read: function read($elem, fieldConfig, config) {
		...
		checkset: {
			...
			draw: function draw(data, blockConfig, config) {
				...
				blockConfig.items.forEach(function (e, idx, a) {
					...
					var $box = $('<input type="checkbox" />').attr('id', itemId).attr('name', e);
					if (id in data && data[id].indexOf(e) > -1) $box.prop('checked', true);
					var $label = $('<label>').attr('for', itemId).text(e);
		...
		text: {
			draw: function draw(data, blockConfig, config) {
				var id = blockConfig.id;
				var $field = $('<input type="text">').attr('name', id).val(data[id]);
		...
		duration: {
			draw: function draw(data, blockConfig, config) {
				...
				return $('<div class="durationField">').attr('name', blockConfig.id).append([$('<input type="number" name="days" />').val(days), ' days ', $('<input type="number" name="hours" />').val(hours), 'hours ', $('<input type="number" name="minutes" />').val(minutes), ' minutes ']);
			},
			read: function read($elem, fieldConfig, config) {
		...
		select: {
			draw: function draw(data, blockConfig, config) {
				...
				if (typeof entries === 'string') entries = this.getPredefinedChoices(entries);
			...
			getPredefinedChoices: function getPredefinedChoices(name) {
				if (name === 'COMPARISON') {
...
//# sourceMappingURL=settingsConsole.js.map
