...
addModule('keyboardNav', function (module, moduleID) {
	...
	module.options = {
		...
		scrollStyle: {
			...
			value: 'directional',
			description: 'When moving up/down with keynav, when and how should RES scroll the window?' + '<br>Directional: Scroll just enough to bring the selected element into view, if it\'s offscreen.' + '<br>Page up/down: Scroll up/down an entire page after reaching the top or bottom.' + '<br>Lock to top: Always align the selected element to the top of the screen.',
			advanced: true
		},
		commentsLinkNumbers: {
			type: 'boolean',
	...
	function registerCommandLine() {
		modules['commandLine'].registerCommand(/\d+/, '[number] - navigates to the link with that number (comments pages) or rank (link pages)', function (command, val, match) {}, function (command, val, match, e) {
	...
	function registerCommand(commandName, spec) {
		...
		module.options[commandName] = $.extend(true, { type: 'keycode' }, module.options[commandName], spec);
	}
	...
	function _moveDown(fromThing) {
		...
		if (!RESUtils.currentSubreddit('dashboard') && RESUtils.isPageType('linklist') && things.index(target) + 2 > things.length && // moving down near the bottom of the list
		modules['neverEndingReddit'].isEnabled() && modules['neverEndingReddit'].options.autoLoad.value) {
	...
	module.showParents = function () {
		...
		if (button) {
			modules['hover'].infocard('showParent').target(button).populateWith(modules['showParent'].showCommentHover).begin();
		}
	};
	...
	module.goMode = function () {
		...
		if (goModeActive) {
			if (!goModePanel) {
				...
				goModePanel = $('<div id="goModePanel" class="RESDialogSmall">').append('<h3>Press a key to go:</h3><div id="goModeCloseButton" class="RESCloseButton">&times;</div>');
