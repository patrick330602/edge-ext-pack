...
addModule('styleTweaks', function (module, moduleID) {
  ...
  */
		...
		if (this.isEnabled() && this.isMatchURL()) {
			...
			if (BrowserDetect.isFirefox()) {
				if (!module.options.subredditStyleBrowserToolbarButton.value) {
					...
					// has been created yet, and if not, wait a bit and try again. There is a miniscule
					// likelihood of ever having to wait, but it is nonzero.
					var setupPageAction = function setupPageAction() {
						if (module.styleToggleCheckbox) {
							RESEnvironment.sendMessage({
								requestType: 'pageAction',
								action: 'show',
	...
	function registerCommandLine() {
		modules['commandLine'].registerCommand('srstyle', 'srstyle [subreddit] [on|off] - toggle subreddit style on/off (if no subreddit is specified, uses current subreddit)', function (command, val) {
	...
	function subredditStyles() {
		...
		if (curSubReddit && subredditTitle) {
			if (modules['nightMode'].isNightModeOn() && !modules['nightMode'].isNightmodeCompatible) {
...
//# sourceMappingURL=styleTweaks.js.map
