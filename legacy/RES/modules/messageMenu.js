...
addModule('messageMenu', function (module, moduleID) {
	...
	var onMouseOver = function onMouseOver(e) {
		modules['hover'].dropdownList(moduleID).target(e.target).options({
	...
	var menuItems;
	var populate = function populate(def, obj, context) {
		if (!menuItems) {
			menuItems = module.options.links.value.map(populateItem).reduce(function (prev, curr) {
				return prev.add(curr);
			}, $());

			menuItems = menuItems.add(populateItem(['<i>' + module.moduleName + '</i>', modules['settingsNavigation'].makeUrlHash(moduleID)]));
		}

		def.resolve(menuItems);
	};

	var populateItem = function populateItem(link) {
		var label = link[0] || '',
		    url = link[1] || '',
		    compose = url.indexOf('/message/compose') !== -1,
		    $link = $('<a />').safeHtml(label).attr('href', url);
