...
addModule('nightMode', function (module, moduleID) {
	...
	module.loadDynamicOptions = function () {
		RESUtils.deferred.all([RESUtils.init.await.headReady(), RESStorage.loadItem('RES_nightMode')]).done(function () {
	...
	module.go = function () {
		if (module.isEnabled() && module.isMatchURL()) {
			...
			modules['commandLine'].registerCommand(/^(ls|ns)$/, 'ns - toggle nightSwitch', function (command, val, match) {
				if (match[1] === 'ls') {
	...
	function handleNightModeStyles() {
		...
		module.isNightmodeCompatible = document.querySelector('.side a[href="#/RES_SR_Config/NightModeCompatible"]') !== null || module.options.useSubredditStyles.value;
	...
	function handleAutomaticNightMode() {
		...
		nightModeOverrideEnd = nightModeOverrideStart ? nightModeOverrideStart + nightModeOverrideLength : 0,
		...
		var needsNightModeToggle = !isOverrideActive && module.isNightModeOn() !== isTimeForNightMode();
	...
	function overrideNightMode() {
		...
		// of override
		RESStorage.setItem('RESmodules.nightMode.nightModeOverrideStart', Date.now());
	}
	...
	module.addSubredditToWhitelist = function (subreddit) {
		...
		RESUtils.options.setOption('nightMode', 'subredditStylesWhitelist', whitelist.join(','));
	};
