...
addModule('saveComments', function (module, moduleID) {
	...
	function addSaveLinks(ele) {
		...
		var allComments = ele.querySelectorAll('div.commentarea > div.sitetable > div.thing div.entry, ' + 'div.commentarea > div.sitetable > div.thing div.entry div.entry');
	...
	function addSaveLinkToComment(commentObj) {
		...
		if ($permaLink.length > 0) {
			...
			if (storedComments && storedComments[saveID]) {
				...
				$saveLink.html('<a class="RES-save noCtrlF" href="javascript:void 0" title="Save using RES - which is local only, but preserves the full text in case someone edits/deletes it" data-text="save-RES"></a>').addClass('saveComments').data('saveID', saveID).data('saveLink', saveHref).data('saveUser', saveUser);
			}
	...
	function saveComment(obj, id, href, username) {
		...
		if (storedComments[id]) {
			...
			if (comment !== null) {
				...
				var $unsaveObj = $('<li>');
				$unsaveObj.html('<a href="javascript:void 0">unsave-RES</a>').data('unsaveID', id).data('unsaveLink', href).addClass('unsaveComments');
	...
	function addSavedCommentsTab() {
		...
		if (mainmenuUL) {
			...
			$('<li id="savedCommentsTab">').html('<a href="javascript:void 0">saved - RES</a>').insertAfter('#savedLinksTab');
	...
	function drawSavedComments() {
		...
		for (var i in storedComments) {
			if (i !== 'RESPro_add' && i !== 'RESPro_delete') {
				...
				$(unsaveLink).data('unsaveID', i).data('unsaveLink', storedComments[i].href);
	...
	module.showEducationalNotification = function () {
		modules['notifications'].showNotification({
			...
			message: '<p>The keyboard shortcuts <b>"' + modules['keyboardNav'].getNiceKeyCode('savePost') + '"</b> (posts) and <b>"' + modules['keyboardNav'].getNiceKeyCode('saveComment') + '"</b> (comments) will save a post/comment to your reddit account (same as the "save" button). It will be accessible from anywhere that you\'re logged in, but the original text will not be preserved if it is edited or deleted.</p>' + '<p>The keyboard shortcut <b>"' + modules['keyboardNav'].getNiceKeyCode('saveRES') + '"</b> will save a comment to RES (same as the "save-RES" button). It will only be available locally, but the original text will be preserved if the comment is edited or deleted.</p>' + '<p>These shortcuts can be changed in the ' + modules['settingsNavigation'].makeUrlHashLink('keyboardNav', 'savePost', 'settings console') + '.<p>'
		});
	};
});
//# sourceMappingURL=saveComments.js.map
