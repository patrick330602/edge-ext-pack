...
addModule('userInfo', function (module, moduleID) {
	...
	function handleMouseOver(e) {
		...
		modules['hover'].infocard(moduleID).target(e.target).options({
			width: 475,
		...
		}).populateWith(showUserInfo).begin();
	}
	...
	function toggleUserHighlightButton(canHighlight) {
		authorInfoToolTipHighlight.toggleClass('blueButton', canHighlight).toggleClass('redButton', !canHighlight).text(canHighlight ? 'Highlight' : 'Unhighlight');
	}
