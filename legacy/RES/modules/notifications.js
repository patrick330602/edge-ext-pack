...
addModule('notifications', function (module, moduleID) {
	...
	module.go = function () {
		if (this.isEnabled() && this.isMatchURL()) {
			modules['commandLine'].registerCommand('notification', false, function () {}, function (command, val, match, e) {
				// test notification
	...
	module.showNotification = function (contentObj, delay) {
		...
		var isSticky = module.options.sticky.value === 'all' || module.options.sticky.value === 'notificationType' && notificationType.sticky;
	...
	function hasNotificationCooledDown(notificationType) {
		...
		for (var i = 0; i < recentNotifications.length; i++) {
			...
			if (current.moduleID == notificationType.moduleID && current.notificationID == notificationType.notificationID) {
