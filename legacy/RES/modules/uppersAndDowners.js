...
addModule('uppersAndDowners', function (module, moduleID) {
	...
	module.include = ['linklist', 'profile', 'comments'];
	...
	module.go = function () {
		if (this.isEnabled() && this.isMatchURL()) {
			if (RESUtils.pageType() === 'comments') {
				applyUppersAndDownersToComments();
				RESUtils.watchForElement('newComments', applyUppersAndDownersToComments);
			} else if (RESUtils.pageType() === 'profile') {
				applyUppersAndDownersToMixed();
				RESUtils.watchForElement('siteTable', applyUppersAndDownersToMixed);
			} else if (RESUtils.pageType() === 'linklist' && this.options.applyToLinks.value) {
				applyUppersAndDownersToLinks();
				RESUtils.watchForElement('siteTable', applyUppersAndDownersToLinks);
			}
		}
	};

	function applyUppersAndDownersToComments(ele) {
		if (!ele) {
			ele = document.body;
		}
		if (ele.classList.contains('comment')) {
			showUppersAndDownersOnComment(ele);
		} else if (ele.classList.contains('entry')) {
			showUppersAndDownersOnComment(ele.parentNode);
		} else {
			var allComments = ele.querySelectorAll('div.comment');
			RESUtils.forEachChunked(allComments, 15, 1000, showUppersAndDownersOnComment);
		}
	}

	function applyUppersAndDownersToMixed(ele) {
		ele = ele || document.body;
		var linkList = ele.querySelectorAll('div.thing.link, div.thing.comment'),
		    displayType = 'regular',
		    thisPlus,
		    thisMinus;
		if (module.options.showSigns.value) {
			thisPlus = '+';
			thisMinus = '-';
		} else {
			thisPlus = '';
		...
		for (var i = 0, len = linkList.length; i < len; i++) {
			if (linkList[i].classList.contains('link')) {
				...
				showUppersAndDownersOnComment(linkList[i]);
			}
		}
	}

	function showUppersAndDownersOnComment(commentEle) {
	...
	function applyUppersAndDownersToLinks(ele) {
...
//# sourceMappingURL=uppersAndDowners.js.map
