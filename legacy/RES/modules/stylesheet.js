...
addModule('stylesheet', function (module, moduleID) {
	...
	module.exclude = ['prefs', 'account', 'subredditAbout'];
	...
	function applyBodyClasses() {
		var addClasses = module.options.bodyClasses.value.filter(function (row) {
			return shouldApply(row[3], row[1], row[2]);
		}).map(function (row) {
			return (row[0] || '').split(/[\s,]/);
		});

		var removeClasses = module.options.bodyClasses.value.filter(function (row) {
			return !shouldApply(row[3], row[1], row[2]);
		}).map(function (row) {
			return (row[0] || '').split(/[\s,]/);
		});
	...
	function loadSubredditStylesheets() {
		var remove = module.options.loadSubredditStylesheets.value.filter(function (row) {
			return !shouldApply(row[3], row[1], row[2]);
		});
		remove = sanitizeSubredditList(remove);

		var add = module.options.loadSubredditStylesheets.value.filter(function (row) {
			return shouldApply(row[3], row[1], row[2]);
		});
	...
	function findSubredditStylesheetElement(findSubreddit) {
		...
		var element = $('link[rel=stylesheet]').filter(function () {
	...
	function createSubredditStylesheetElement(subreddit) {
		var element = $('<link rel="stylesheet">').data('res-stylesheet', subreddit).attr('href', document.location.origin + '/r/' + subreddit + '/stylesheet.css');
		return element;
	}
	...
	function applyCssSnippets() {
		...
		var addElements = module.options.snippets.value.filter(function (row) {
			return shouldApply(row[3], row[1], row[2]) && findSnippetElement(row[0]).length === 0;
		}).map(function (row) {
		...
		var removeElements = module.options.snippets.value.filter(function (row) {
			return !shouldApply(row[3], row[1], row[2]);
		}).map(function (row) {
