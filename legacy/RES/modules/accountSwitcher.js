...
addModule('accountSwitcher', function (module, moduleID) {
				...
				if (accounts !== null) {
					...
					accounts.forEach(function (account) {
						...
						if (!loggedInUser || username.toUpperCase() !== loggedInUser.toUpperCase() || this.options.showCurrentUserName.value) {
							...
							$accountLink.data('username', username).appendTo(this.accountMenu);
						}
					}, this);
					...
					$('<li>', {
						'class': 'addAccount'
					}).text('+ add account').appendTo(this.accountMenu);
				}

				$(document.body).append(this.accountMenu);
			}

			modules['commandLine'].registerCommand(/^sw$/, 'sw [username] - switch users to [username]', function (command, val, match) {
	...
	function populateUsers() {
		module.accountMenu.find('.accountName').each(function (index, ele) {
			populateUser(ele);
		});
	}

	function updateUser(username) {
		module.accountMenu.find('.accountName').filter(function () {
			...
			RESEnvironment.ajax({
				...
				onload: function onload(response) {
					...
					if (badData) {
						...
						alert('Could not log in as <b>' + username + '</b> because reddit is seeing too many requests from you too quickly. Perhaps you\'ve been trying to log in using the wrong username or password?' + '<p>Check your settings?</p>', manageAccounts);
						switchedAccount(false);
					} else {
	...
	function _switchedAccountElsewhere() {
		...
		hasDraft = Array.prototype.slice.call(document.querySelectorAll('textarea')).some(function (textarea) {
		...
		switchedMessage = username ? 'You switched to /u/' + username + '.' : 'You have been logged out.';
		reloadMessage = hasDraft ? 'However, you haven\'t finished posting on this page as /u/' + RESUtils.loggedInUser() + '. Do you want to reload the page anyway?' : 'Reload this page now?';
