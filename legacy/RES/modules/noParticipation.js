...
addModule('noParticipation', function (module, moduleID) {
	...
	module.include = [/^https?:\/\/(?:.*\.)?(?:\w+-)?np(?:-\w+)?\.reddit\.com\/*/i // np.reddit.com, np-nm.reddit.com, nm-np.reddit.com, www.np.reddit.com, www.np-nm.reddit.com
	];
	...
	function notifyNoVote(voteButton) {
		...
		var notification = modules['notifications'].showNotification({
			...
			cooldown: 5000,
			header: 'No Participation',
			message: '<strong><span class="res-icon">&#xF15A;</span> Do not vote.</strong>' + boilerplateNotificationText + (canUndoVote ? '<p><button type="button" class="redButton" data-np="revertvote">Undo vote</button></p>' : '')
		});
...
//# sourceMappingURL=noParticipation.js.map
