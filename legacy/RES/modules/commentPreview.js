...
addModule('commentPreview', function (module, moduleID) {
	...
	module.include = ['comments', 'inbox', 'submit', 'profile', /^https?:\/\/(?:[\-\w\.]+\.)?reddit\.com\/r\/[\-\w\.]+\/about\/(?:edit|modqueue|reports|spam|banned)/i, 'wiki'];
	module.exclude = [/^https?:\/\/(?:[\-\w\.]+\.)?reddit\.com\/(?:r\/[\-\w\.]+\/)?wiki\/edit\/config\/automoderator\b/i];
   ...
   */
	...
	function attachPreview(usertext) {
		...
		[$(usertext).find('.usertext-edit'), $(usertext).find('.ban-details:has(#ban_message)')].forEach(function (elements) {
			elements.each(function() {
				var $this = $(this);

				if (!module.options.enableForComments.value && $this.closest('.commentarea, .message').length) {
					return;
				}

				if (!module.options.enableForPosts.value && (RESUtils.pageType() === 'submit' || $(this).closest('.link').length)) {
					return;
				}

				if (!module.options.enableForSubredditConfig.value && /^https?:\/\/(?:[\-\w\.]+\.)?reddit\.com\/r\/[\-\w\.]+\/about\/edit/i.test(document.location.href)) {
					return;
				}

				if (!module.options.enableForBanMessages.value && /^https?:\/\/(?:[\-\w\.]+\.)?reddit\.com\/r\/[\-\w\.]+\/about\/banned/i.test(document.location.href)) {
					return;
				}
				...
				if (module.options.sidebarPreview.value && textareas.attr('name') === 'description') {
	...
	function attachWikiPreview() {
		...
		if (module.options.enableForWiki.value && RESUtils.pageType() === 'wiki') {
	...
	function showBigEditor(e) {
		...
		dialogContentBox.style.paddingBottom = dialogTitle.offsetHeight - 10 + // add offsetHeight and subtract margin
		editorWrapperHeight + dialogFooter.offsetHeight + 'px';
	}
...
//# sourceMappingURL=commentPreview.js.map
