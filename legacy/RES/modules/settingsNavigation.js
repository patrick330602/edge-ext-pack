...
addModule('search', function (module, moduleID) {
	...
	module.go = function () {
		...
		modules['commandLine'].registerCommand(/^set(?:t?ings?)?$/, 'settings [words to search for]- search RES settings console', function (command, val) {
	...
	function rankString(queryTerms, string) {
		...
		var reduced = weighted.length ? weighted.reduce(function (a, b) {
			return a + b;
		}, 0) : Infinity;
		return reduced;
	}
	...
	function makeOptionSearchResult(moduleKey, optionKey) {
		...
		var result = {};
		result.type = 'option';
		result.breadcrumb = ['RES settings console', [].concat(mod.category)[0], mod.moduleName + ' (' + mod.moduleID + ')'].join(' > ');
	...
	function makeModuleSearchResult(moduleKey) {
		...
		var result = {};
		result.type = 'module';
		result.breadcrumb = ['RES settings console', [].concat(mod.category)[0], '(' + mod.moduleID + ')'].join(' > ');
	...
	function drawSearchResultItem(result) {
		var element = $('<li>');
		element.addClass('SearchRES-result-item').toggleClass('advanced', !!result.advanced).data('SearchRES-result', result);

		$('<span>', {
			class: 'SearchRES-result-copybutton res-icon'
		}).html('&#xF159').appendTo(element).attr('title', 'copy this for a comment');
...
//# sourceMappingURL=settingsNavigation.js.map
