...
addModule('hover', function (module, moduleID) {
	...
	module.instance = function getInstance(type, id, hidden) {
		...
		if (!instances[type][id]) {
			instances[type][id] = type === 'dropdownList' ? new HoverDropdownList(id) : type === 'infocard' ? new HoverInfoCard(id) : new Hover(id);
	...
	var isEnabled = function isEnabled(type, id) {
		var row = findOrMakeRow(type, id);
		return row[1];
	};

	var findOrMakeRow = function findOrMakeRow(type, id) {
		var name = type + '.' + id;
	...
	var _updatePositionOnScroll2 = function _updatePositionOnScroll() {
		...
		_updatePositionOnScroll2 = function _updatePositionOnScroll() {};
	};
	...
	Hover.prototype = {
		...
		_construct: function _construct(id) {
			this.instanceID = id;
			this._bindAllMethods();
			this.options({});
		},
		_bindAllMethods: function _bindAllMethods() {
			for (var key in this) {
		...
		options: function options(_options) {
			if (typeof _options !== 'undefined') {
				this._options = $.extend(true, {}, this._options, _options);
			}
		...
		target: function target(element) {
		...
		_container: null,
		getContainer: function getContainer(callback) {
			if (!this._container) {
		...
		_addContainerHandlers: function _addContainerHandlers($container) {
			$container.appendTo(document.body).on('mouseenter', function () {
			...
			}.bind(this)).on('click', '.RESCloseButton', function () {
				this.close(true);
			}.bind(this));
		},
		...
		open: function open() {
			...
			this.getContainer(function ($container) {
				if (this._callback) {
					var def = $.Deferred();
					def.promise().progress(this._populate.bind(this, $container)).done(this._populate.bind(this, $container)).fail(function () {
						this.close();
					});
		...
		_populate: function _populate($container /*,  ...contents*/) {
			if (!this._enabled) return false;
		...
		_updatePosition: function _updatePosition($container) {
		...
		cancelShow: function cancelShow(fade) {
			fade = typeof fade === 'boolean' ? fade : false;
			this.close(fade);
		},
		addShowListeners: function addShowListeners() {
			$(this._target)
			// .on('click', this.cancelShow)
			.on('mouseleave', this.cancelShow);
		},
		clearShowListeners: function clearShowListeners() {
			$(this._target)
			// .off('click', this.cancelShow)
			.off('mouseleave', this.cancelShow);
		},
		startShowTimer: function startShowTimer() {
		...
		cancelShowTimer: function cancelShowTimer() {
			clearTimeout(this.showTimer);
			this.showTimer = null;
		},
		afterShowTimer: function afterShowTimer() {
			this.cancelShowTimer();
		...
		afterHideTimer: function afterHideTimer() {
			this.cancelHideTimer();
			this.close(true);
			this.visible = false;
		},
		close: function close(fade) {
			if (!this._enabled) return false;
