...
addModule('selectedEntry', function (module, moduleID) {
	...
	module.go = function () {
		...
		if (module.options.autoSelectOnScroll.value) {
			window.addEventListener('scroll', function (e) {
				RESUtils.debounce(moduleID + '.autoSelectOnScroll', 300, onScroll);
			});
		}
	...
	function scrollTo(selected, last, options) {
		if (!selected) return;
		options = $.extend(true, { makeVisible: selected.entry }, options);
		RESUtils.scrollToElement(selected.thing, options);
	}

	function addSelectListeners() {
		addListener(function (selected, unselected) {
			if (unselected) modules['hover'].infocard('showParent').close(false);
		});
...
//# sourceMappingURL=selectedEntry.js.map
