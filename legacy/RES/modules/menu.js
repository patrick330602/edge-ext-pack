...
addModule('RESMenu', function (module, moduleID) {
	...
	function renderConsoleLink() {
		var RESPrefsLink = $('<span id="openRESPrefs"><span id="RESSettingsButton" title="RES Settings" class="gearIcon"></span>').mouseenter(module.showPrefsDropdown.bind(module));
	...
	module.showPrefsDropdown = function () {
		modules['hover'].dropdownList(moduleID, modules['hover'].HIDDEN_FROM_SETTINGS).options({
			openDelay: 200,
			fadeDelay: 200,
			fadeSpeed: 0.2
		}).populateWith(populateDropdown).target(module.RESPrefsLink.querySelector('#RESSettingsButton')) // workaround subreddit stylings where the container ends up super tall
		.begin();
	};
