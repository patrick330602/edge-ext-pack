...
;function enableTranslation(){chrome.tabs.query({active:!0,currentWindow:!0},function(a){var b=a[0].id;chrome.tabs.detectLanguage(b,function(a){var c=Microsoft.JS.TextLanguage.normalizeCode(a);Logger.info("Language detection done.",{tabId:b,lang:a,normalizedLang:c}),Microsoft.JS.MessageClient.enableTranslation(b,c).then(function(){return Microsoft.JS.injectTranslatorScripts(b,c,translateTo)}).then(function(){window.close()}).catch(function(a){translateButton.style.backgroundColor="red",Logger.error("Failed to enable translation",a)})})})}function disableTranslation(){chrome.tabs.query({active:!0,currentWindow:!0},function(a){var b=a[0].id;Microsoft.JS.MessageClient.disableTranslation(b).then(function(){return Microsoft.JS.injectDisableTranslatorScripts(b)}).then(function(){window.close()}).catch(function(a){showOriginalButton.style.backgroundColor="red",Logger.error("Failed to disable translation",a)})})}function localStoreGet(a){return new Promise(function(b,c){chrome.storage.local.get(a,function(a){chrome.runtime.lastError?c(chrome.runtime.lastError):b(a)})})}function getTabId(){return new Promise(function(a,b){chrome.tabs.query({active:!0,currentWindow:!0},function(c){chrome.runtime.lastError?b(chrome.runtime.lastError):a(c[0].id)})})}document.getElementById("optionsLink").onclick=function(a){a.preventDefault();var b=window.location.href;b=b.substring(0,b.lastIndexOf("/")+1)+this.getAttribute("href"),chrome.tabs.create({url:b})};var tabId=-1,alwaysTranslate=!1,tabInfo={},translateTo=getUiTextLang(),translateButton=document.getElementById("translateButton");Promise.all([getTabId(),localStoreGet(["alwaysTranslate","tabInfo","translateTo"])]).then(function(a){tabId=a[0],a[1].tabInfo&&(tabInfo=JSON.parse(a[1].tabInfo)[tabId]||{}),a[1].alwaysTranslate!==undefined&&(alwaysTranslate=a[1].alwaysTranslate),String.isNullOrWhiteSpace(a[1].translateTo)||(translateTo=a[1].translateTo),document.getElementById("checkbox").checked=alwaysTranslate,tabInfo.translationEnabled===!0?(translateButton.innerHTML=chrome.i18n.getMessage("showOriginalButton"),translateButton.onclick=disableTranslation):(translateButton.innerHTML=chrome.i18n.getMessage("translateButton"),translateButton.onclick=enableTranslation)}).catch(function(a){Logger.error("Failed to get tab ID and/or values from storage",a)})
