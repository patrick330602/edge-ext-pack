...
(function ($) {
    ...
    var doc_canvas_copy = null;

    window.updateDocumentCanvasCopy = function (callback) {
        html2canvas(document.body, {
            onrendered: function (doc_canvas) {
                doc_canvas_copy = doc_canvas;
                (callback || function () {
                })(doc_canvas);
            },
    ...
    $.fn.blur = function (options) {
        ...
        var commitBluring = function () {
            that.each(function () {
                ...
                var ctx = canvas.getContext('2d');
                ctx.drawImage(doc_canvas_copy, x, y, width, height, 0, 0, width, height);

                canvas.style.filter = 'blur(' + options.radius + 'px)';
        ...
        if (doc_canvas_copy !== null && !options.update) {
            commitBluring();
        } else {
            $(this).attr('data-html2canvas-ignore', 'true');
            updateDocumentCanvasCopy(function () {
                commitBluring();
                $(that).removeAttr('data-html2canvas-ignore');
            });
        }
    };
})(jQuery);